...
HMODULE CStealthInject::GetRemoteModuleHandle(HANDLE hProcess, const char* module)
{
...
FARPROC CStealthInject::RemoteGetProcAddress(HANDLE hProcess, HMODULE hModule, const char* func)
{
	// Main DLL we will need to load
	HMODULE kernel32 = NULL;
...
DWORD CStealthInject::Inject(HANDLE hProcess, const char* dllname)
{
	// Main DLL we will need to load
	HMODULE kernel32 = NULL;
...
bool CStealthInject::ReadMemory(HANDLE hProcess, LPVOID baseAddress, LPVOID buffer, int size)
{
	static DWORD bytesRead;
	return ReadProcessMemory(hProcess, baseAddress, buffer, size, &bytesRead) == TRUE;
}

bool CStealthInject::WriteMemory(HANDLE hProcess, LPVOID baseAddress, LPVOID buffer, int size)
{
	static DWORD bytesWritten;
	return WriteProcessMemory(hProcess, baseAddress, buffer, size, &bytesWritten) == TRUE;
}

void CStealthInject::LoadSections(HANDLE hProcess)
{
...
void CStealthInject::Relocate(HANDLE hProcess, int delta)
{
...
void CStealthInject::ResolveImports(HANDLE hProcess)
{
...
bool CStealthInject::StealthLoadLibraryMemory(HANDLE hProcess, void* address)
{
	...
	if (_injectingNtHeaders.OptionalHeader.AddressOfEntryPoint)
	{
		...
		DWORD exitThread = (DWORD)GetProcAddress(GetModuleHandle(_T("kernel32")), "ExitThread");
...
void CStealthInject::StealthLoadLibrary(void* hProcess, const char* path)
{
