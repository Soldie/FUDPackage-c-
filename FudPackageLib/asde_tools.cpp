...
int _iswow64() {
 ...
 int _exists_regkey(HKEY hKey, char * regkey_s) {
	HKEY regkey;
	LONG ret;
...
int _exists_regkey_value_str(HKEY hKey, char * regkey_s, char * value_s, char * lookup) {
	HKEY regkey;
	LONG ret;
	DWORD size;
...
int wmi_initialize(const wchar_t *query_namespace, IWbemServices **services)
{
	...
	_namespace = SysAllocString(query_namespace);

	// Connect to the namespace with the current user and obtain pointer
	// services to make IWbemServices calls.
	hresult = locator->ConnectServer(_namespace, NULL, NULL, NULL, 0,NULL, NULL, services);
...
int wmi_check_query(IWbemServices *services, const wchar_t *language, const wchar_t *query,
	...
	HRESULT result = services->ExecQuery(
		 wmilang, wmiquery, WBEM_FLAG_BIDIRECTIONAL, NULL, &queryrows);
